> HISTORICAL RECORD - superseded by `RELEASE-1.0.1-STATUS.md`. The production origin is no longer hard-coded.
...
- `https://ISOLATED-MATURITA-ORIGIN.example` — připnutý izolovaný serverless produkční origin pro podepsaný CONFIDENTIAL-EXAM;
...
1. Zřídit a ověřit custom origin `ISOLATED-MATURITA-ORIGIN.example` na schváleném statickém hostingu.
