...
const required = [
  ...
  'tools/generate-publisher-key.mjs','tools/sign-content-pack.mjs','tools/verify-content-pack-signature.mjs','tools/create-content-pack.mjs','tools/create-synthetic-demo-pack.mjs','tools/create-stage13-stress-pack.mjs',
  ...
  'samples/synthetic-demo-2027.mdesk','README.md','RELEASE-1.0.0-STATUS.md','SERVERLESS-PRODUCTION-DEPLOY.txt','DEVICE-ACCEPTANCE-1.0.0.txt','GITHUB-UPDATE-1.0.0.txt','SECURITY-NOTES.md','BUILD-REPORT.md',
...
if (pkg.version !== '1.0.0') failures.push('Final serverless version must be 1.0.0');
...
if (consumer.cache?.name !== 'ghrab-maturita-desk-v1.0.0' || manifest.ghrab_platform?.cache_name !== 'ghrab-maturita-desk-v1.0.0' || !sw.includes("ghrab-maturita-desk-v1.0.0")) failures.push('1.0.0 cache version mismatch');
if (platform.stage !== 'serverless-1.0.0' || consumer.quality?.stage !== 'serverless-1.0.0') failures.push('Final serverless stage marker missing');
...
if (!deployment.allowedOrigins?.includes('https://maturita-fact.ghrabuvka.cz')) failures.push('Pinned serverless Fact Check origin missing');
if (!deployment.trust?.appOrigins?.includes('https://daniel22-dev.github.io') || !deployment.trust?.appOrigins?.includes('https://maturita.ghrabuvka.cz')) failures.push('App origin pins incomplete');
if (JSON.stringify(deployment.trust?.confidentialContentOrigins) !== JSON.stringify(['https://maturita.ghrabuvka.cz'])) failures.push('CONFIDENTIAL-EXAM must be limited to the isolated production origin');
...
if (!baked.includes("confidentialContentOrigins: Object.freeze(['https://maturita.ghrabuvka.cz'])") || !baked.includes("'ghrab-maturita-content-2026-01'")) failures.push('Baked release trust anchor incomplete');
...
if (serverQa.serverConnection?.liveSchoolServerConnected !== false) failures.push('1.0.0 must not falsely claim a live school server');
...
if (!runtime.includes('confidentialOriginAllowed') || !runtime.includes('narrowPublicKeys') || !runtime.includes('publisherKeys')) failures.push('Runtime release trust for confidential content incomplete');
...
if (!main.includes("APP_VERSION = '1.0.0'") || !main.includes('Diagnostika zařízení') || !main.includes('Ověřit / dohledat') || !main.includes('Podpis vydavatele') || !main.includes('RUNTIME_CONFIG.content.confidentialAllowed')) failures.push('Final serverless UX incomplete');
...
if (!diagnostics.includes("PILOT_BUILD = 'serverless-1.0.0'") || !diagnostics.includes('PILOT_SYNTHETIC_ONLY = false') || !diagnostics.includes('SERVERLESS DEVICE DIAGNOSTICS')) failures.push('Device diagnostics model incomplete');
...
if (!sw.includes('isProtectedOrRuntime') || !sw.includes("relative.endsWith('.mdesk')") || !sw.includes("'./src/content-pack.js'")) failures.push('Service Worker protected-content cache policy regression');
for (const module of ['./src/providers/runtime.js','./src/providers/auth-provider.js','./src/providers/content-provider.js','./src/providers/registry.js','./src/device-runtime.js','./src/pilot.js','./src/session-coordinator.js']) if (!sw.includes(module)) failures.push(`Offline shell module missing: ${module}`);
...
if (!index.includes('Content-Security-Policy') || !index.includes("script-src 'self'") || !index.includes("script-src-attr 'none'") || !index.includes("object-src 'none'") || !index.includes("connect-src 'self' https://maturita-fact.ghrabuvka.cz")) failures.push('Static CSP profile missing or overly broad');
...
if (failures.length) {
  console.error('Maturita Desk 1.0.0 serverless validation: FAIL');
  failures.forEach(item => console.error(`- ${item}`));
  process.exit(1);
}
console.log('Maturita Desk 1.0.0 serverless validation: PASS');
console.log(`Version: ${pkg.version}`);
console.log('Public shell: no real exam content, passphrase, OpenAI secret or publisher private key.');
console.log('Origin policy: shared GitHub Pages = demo-only; pinned isolated production origin = signed CONFIDENTIAL-EXAM allowed.');
