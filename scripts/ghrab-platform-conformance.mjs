...
assert.equal(consumer.appVersion, '1.0.2');
...
assert.equal(consumer.cache.name, 'ghrab-maturita-desk-v1.0.2');
...
assert.equal(build.appVersion, '1.0.2');
...
assert.match(index, /data-ghrab-app-version="1\.0\.2"/);
assert.match(index, /src\/platform-config\.js/);
assert.match(index, /ghrab\/ghrab-platform\.js\?v=1\.0\.2/);
assert.match(index, /ghrab\/ghrab-platform\.css\?v=1\.0\.2/);
